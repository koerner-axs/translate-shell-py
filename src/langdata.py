...
def show_translations_of(code, text):
    """Return formatted text for "translations of"."""
    locale_code = get_code(code)
    fmt = None
    if locale_code and locale_code in LOCALES:
        fmt = LOCALES[locale_code].get("translations-of")
    if not fmt and "en" in LOCALES:
        fmt = LOCALES["en"].get("translations-of")
    if fmt:
        return fmt % text
    return text


def show_definitions_of(code, text):
    """Return formatted text for "definitions of"."""
    locale_code = get_code(code)
    fmt = None
    if locale_code and locale_code in LOCALES:
        fmt = LOCALES[locale_code].get("definitions-of")
    if not fmt and "en" in LOCALES:
        fmt = LOCALES["en"].get("definitions-of")
    if fmt:
        return fmt % text
    return text
